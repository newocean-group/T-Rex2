...
def build_swin_transformer(model_name, pretrain_img_size, **kw):
    ...
    kw_cgf = model_para_dict[model_name]
    kw_cgf.update(kw)
...
def build_backbone(model_name, pretrain_img_size):
    ...
    inputs = torch.randn(size=(1, 3, 336, 336))
    inputs_mask = torch.ones_like(inputs)[:, 0]
    tensor_list = NestedTensor(inputs, inputs_mask)
    outs: dict = backbone(tensor_list)
    out_channels = [out[1].tensors.shape[1] for out in outs.items()]
