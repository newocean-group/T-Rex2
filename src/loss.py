...
class InfoNCE(nn.Module):
    ...
    def forward(self, im_embeds, txt_embeds):
        """
        im_emb shape: K x c
        txt_emb shape: K x c
        """
        n_prompts = im_embeds.shape[0]

        # L2 normalize the embeddings
        im_embeds = F.normalize(im_embeds, p=2, dim=-1)
        txt_embeds = F.normalize(txt_embeds, p=2, dim=-1)

        # Compute similarity matrix
        logits = (im_embeds @ txt_embeds.T) / self.temperature

        # Create labels (0...batch_size-1) for positive pairs
        labels = torch.arange(n_prompts, device=logits.device, dtype=torch.int64)
